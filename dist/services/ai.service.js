...
class AIService {
    async getRecommendedUsers(userId, user) {
        try {
            const query = {
                _id: { $ne: userId },
                isActive: true,
            ...
            const userList = users.map((u) => {
                ...
                if (u.occupation)
                    parts.push(`| ${u.occupation}`);
                if (u.location?.city)
                    parts.push(`| ${u.location.city}, ${u.location.state || ''}`);
                if (u.interests?.length)
                    parts.push(`| Interests: ${u.interests.slice(0, 5).join(', ')}`);
                if (u.lifestyle?.sleepSchedule)
                    parts.push(`| Sleep: ${u.lifestyle.sleepSchedule}`);
                if (u.lifestyle?.cleanliness)
                    parts.push(`| Cleanliness: ${u.lifestyle.cleanliness}/5`);
                if (u.lifestyle?.socialLevel)
                    parts.push(`| Social: ${u.lifestyle.socialLevel}/5`);
                if (u.preferences?.budget)
                    parts.push(`| Budget: ₦${u.preferences.budget.min?.toLocaleString()}-₦${u.preferences.budget.max?.toLocaleString()}/yr`);
                if (u.preferences?.roomType)
                    parts.push(`| Room: ${u.preferences.roomType}`);
                if (u.verified)
                    parts.push(`| ✓ Verified`);
                if (u.bio)
                    parts.push(`| Bio: "${u.bio.substring(0, 80)}"`);
                return parts.join(' ');
            }).join('\n');
            ...
            logger_1.default.warn('Failed to load recommended users for AI:', e);
            return '';
        }
    }
    async getAvailableListings(user) {
        try {
            ...
            let listings = await models_1.Property.find(query)
            ...
            if (!listings.length && user?.location?.city) {
                listings = await models_1.Property.find({ status: 'available' })
                    .select('title type price location bedrooms bathrooms amenities furnished availableFrom leaseDuration')
                    .sort({ createdAt: -1 })
                    .limit(10)
                    .lean();
            ...
            const listingList = listings.map((l) => {
                ...
                if (l.location?.city)
                    parts.push(`| ${l.location.city}, ${l.location.state || ''}`);
                parts.push(`| ${l.bedrooms} bed, ${l.bathrooms} bath`);
                if (l.furnished)
                    parts.push(`| Furnished`);
                if (l.amenities?.length)
                    parts.push(`| Amenities: ${l.amenities.slice(0, 5).join(', ')}`);
                if (l.leaseDuration)
                    parts.push(`| Lease: ${l.leaseDuration} months`);
                return parts.join(' ');
            }).join('\n');
            ...
            logger_1.default.warn('Failed to load listings for AI:', e);
            return '';
        }
    }
    async chat(messages, userId) {
        ...
        try {
            currentUser = await models_1.User.findById(userId).select('firstName lastName location preferences lifestyle occupation interests gender dateOfBirth gamification verified subscription blockedUsers').lean();
            if (currentUser) {
                const parts = [];
                if (currentUser.firstName)
                    parts.push(`Name: ${currentUser.firstName} ${currentUser.lastName || ''}.`);
                if (currentUser.gender)
                    parts.push(`Gender: ${currentUser.gender}.`);
                ...
                if (currentUser.occupation)
                    parts.push(`Occupation: ${currentUser.occupation}.`);
                if (currentUser.location?.city)
                    parts.push(`Location: ${currentUser.location.city}, ${currentUser.location.state || 'Nigeria'}.`);
                if (currentUser.preferences?.budget)
                    parts.push(`Budget: ₦${currentUser.preferences.budget.min?.toLocaleString()} - ₦${currentUser.preferences.budget.max?.toLocaleString()}/year.`);
                if (currentUser.preferences?.roomType)
                    parts.push(`Room preference: ${currentUser.preferences.roomType}.`);
                if (currentUser.interests?.length)
                    parts.push(`Interests: ${currentUser.interests.slice(0, 8).join(', ')}.`);
                if (currentUser.lifestyle?.sleepSchedule)
                    parts.push(`Sleep: ${currentUser.lifestyle.sleepSchedule}.`);
                if (currentUser.lifestyle?.cleanliness)
                    parts.push(`Cleanliness: ${currentUser.lifestyle.cleanliness}/5.`);
                if (currentUser.lifestyle?.socialLevel)
                    parts.push(`Social level: ${currentUser.lifestyle.socialLevel}/5.`);
                if (currentUser.lifestyle?.guestFrequency)
                    parts.push(`Guest frequency: ${currentUser.lifestyle.guestFrequency}.`);
                if (currentUser.lifestyle?.workFromHome)
                    parts.push(`Works from home.`);
                if (currentUser.gamification)
                    parts.push(`Points: ${currentUser.gamification.points?.toLocaleString() || 0}. Level: ${currentUser.gamification.level || 1}.`);
                if (currentUser.verified)
                    parts.push(`Account verified.`);
                if (currentUser.subscription?.plan)
                    parts.push(`Subscription: ${currentUser.subscription.plan}.`);
                // Load clan info
                try {
