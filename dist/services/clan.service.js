...
class ClanService {
            ...
            if (accept) {
                ...
                if (challenger && defender) {
                    ...
                    try {
                        ...
                        logger_1.default.warn('War match notification error:', e);
                    }
                    // Emit socket event to challenger clan members
                    try {
                        const memberIds = challenger.members.map((m) => m.user.toString());
                        (0, socket_config_1.emitClanWarUpdate)(memberIds, 'clan:war_accepted', { warId: war._id.toString(), status: 'in_progress' });
                    }
                    catch { }
                }
            }
     ...
     */
    async cancelWar(clanId, warId, userId) {
        try {
            const war = await Clan_1.ClanWar.findById(warId);
            if (!war)
                throw new Error('War not found.');
            ...
            if (!isChallenger && !isDefender)
                throw new Error('Your clan is not part of this war.');
            const clan = await Clan_1.Clan.findById(clanId);
            if (!clan)
                throw new Error('Clan not found.');
            ...
            const warShield = (clan.purchasedUpgrades || []).find(p => p.itemId === 'war_shield_24h' && p.expiresAt && p.expiresAt > new Date());
            const CANCEL_COST = 200;
            if (warShield) {
                logger_1.default.info(`Clan ${clan.tag} used War Shield to cancel war ${warId} for free`);
            }
            else {
            ...
            try {
                const otherClan = await Clan_1.Clan.findById(otherClanId);
                if (otherClan) {
                    await notification_service_1.default.createNotification({
                    ...
                    try {
                        ...
                        (0, socket_config_1.emitClanWarUpdate)(memberIds, 'clan:war_declined', { warId: war._id.toString(), status: 'declined' });
                    }
                    catch { }
                }
            }
            catch (e) {
                logger_1.default.warn('War cancel notification error:', e);
            }
            logger_1.default.info(`Clan ${clan.tag} cancelled war ${warId}`);
            return war;
        }
        catch (error) {
            logger_1.default.error('cancelWar error:', error);
            throw error;
        }
    }
    /**
