...
const StudyQuestion_1 = require("../models/StudyQuestion");
...
function generateVariations(baseQuestions, category) {
...
async function seed() {
    ...
    for (const category of [...allCategories, ...additionalCategories]) {
        const existing = await StudyQuestion_1.StudyQuestion.countDocuments({ category });
        if (existing >= 400) {
        ...
        const existingQuestions = await StudyQuestion_1.StudyQuestion.find({ category }).select('question').lean();
        ...
        await StudyQuestion_1.StudyQuestion.insertMany(docs, { ordered: false }).catch(() => { });
...
//# sourceMappingURL=seed-study-questions.js.map