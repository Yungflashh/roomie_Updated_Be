...
class CompatibilityService {
  ...
  calculateCompatibility(user1: IUserDocument, user2: IUserDocument): number {
    ...
    const totalScore = 
      factors.budgetScore * weights.budgetScore +
   ...
   */
  private calculateFactors(user1: IUserDocument, user2: IUserDocument): CompatibilityFactors {
    return {
  ...
  private calculateLifestyleScore(user1: IUserDocument, user2: IUserDocument): number {
    ...
    if (lifestyle1.sleepSchedule && lifestyle2.sleepSchedule) {
      ...
      if (lifestyle1.sleepSchedule === lifestyle2.sleepSchedule) {
        ...
        lifestyle1.sleepSchedule === 'flexible' ||
    ...
    // Work from home
    if (
      typeof lifestyle1.workFromHome === 'boolean' &&
  ...
  private calculatePreferencesScore(user1: IUserDocument, user2: IUserDocument): number {
    ...
    if (pref1.gender && pref2.gender && user1.gender && user2.gender) {
      factors++;
      const user1MatchesPref2 =
        pref2.gender === 'any' || user1.gender === pref2.gender;
      const user2MatchesPref1 =
        pref1.gender === 'any' || user2.gender === pref1.gender;
    ...
    // Pet friendly
    if (
      typeof pref1.petFriendly === 'boolean' &&
   ...
   */
  private calculateAgeScore(user1: IUserDocument, user2: IUserDocument): number {
    if (!user1.dateOfBirth || !user2.dateOfBirth) return 50;
