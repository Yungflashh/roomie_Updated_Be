...
import { StudySession } from '../models/StudyBuddy';
...
const QUESTION_BANK: Record<string, Array<{ question: string; options: string[]; correctAnswer: string; explanation: string }>> = {
  'computer-science': [
...
class StudyBuddyService {
   ...
   */
  async findStudyBuddies(userId: string, category: string) {
    try {
      ...
      logger.error('Find study buddies error:', error);
   ...
   */
  async createSoloSession(userId: string, category: string, questionCount: number = 10) {
    try {
   ...
   */
  async createChallengeSession(userId: string, opponentId: string, category: string, questionCount: number = 10) {
    try {
   ...
   */
  async respondToChallenge(sessionId: string, userId: string, accept: boolean) {
    try {
   ...
   */
  async submitAnswers(sessionId: string, userId: string, answers: Array<{ questionIndex: number; answer: string; timeSpent: number }>) {
    try {
      const session = await StudySession.findById(sessionId);
      if (!session) {
        throw new Error('Session not found');
      ...
      if (creatorDone && opponentDone) {
        ...
        if (winnerId) completionUpdate.winner = winnerId;

        const completed = await StudySession.findOneAndUpdate(
   ...
   */
  async getLeaderboard(category: string, limit: number = 20) {
    try {
   ...
   */
  private async getRandomQuestions(category: string, count: number) {
