...
class GameController {
   ...
   */
  async sendMultiplayerInvitation(req: AuthRequest, res: Response): Promise<void> {
    try {
      const userId = req.user?.userId!;
      ...
      const session = await gameService.sendMultiplayerInvitation(gameId, userId, invitees);
      ...
      logger.error('Send multiplayer invitation error:', error);
      res.status(400).json({
        success: false,
